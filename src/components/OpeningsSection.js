...
const OpeningsSection = () => {
  ...
  useEffect(() => {
    const fetchJobOpenings = async () => {
      try { setJobOpenings(await listContent('job_openings')); }
  ...
  return (
    <section className="py-12">
      <div className="container mx-auto">
        ...
        {jobOpenings.length === 0 ? <p className="text-center text-gray-500">No job openings available yet.</p> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {jobOpenings.map((job) => (
